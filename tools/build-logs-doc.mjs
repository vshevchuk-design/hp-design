...
const POSTS = [
  {
    ...
    body: "The staff prototype's split-pane chat is fully reworked into an email inbox. Console: a persistent topbar (mail + department + a SplitButton \"New Message ▾\" + user avatar) over a full-width Table (Student / Subject & Message / Responsible / Expiration / Date), unread rows bold; on mobile the table reflows to stacked list rows. Opening a thread is a full-page detail (topbar kept, Gmail-style) with inline read receipts (\"Seen · time\") and a Reply / Reply & Resolve composer. \"Responsible\" replaces Handled-by/Unassigned. The lost Group Message flow is back: a Stepper wizard (select students by add/paste-IDs → message details with a rich editor, AI Assist, and a DatePicker) that fans out to a grouped card in Resolved; opening it shows delivery Stat tiles + a replies list + a recipients Drawer with status filters. Built on the seven new components (Table / AvatarGroup / Stepper / DatePicker / SplitButton / Stat / Skeleton); the compose dialog + AI panel + rich composer carry over.",
    links: [
